...
from researchmind.agent.tools import (
    read_session_memory,
    search_corpus,
    search_recent,
    synthesise_answer,
...
def build_graph(
    ...
    builder.add_node(
        "trace_citation_graph",
        partial(
            trace_citation_graph, retriever=retriever, llm=llm, graph=citation_graph
        ),
    )
    builder.add_node(
        "compare_methodologies",
        partial(compare_methodologies, retriever=retriever, llm=llm),
    )
    builder.add_node(
        "detect_research_gaps",
