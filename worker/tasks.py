...
from researchmind.ingestion.download import HttpPDFDownloader
from researchmind.ingestion.parsing import PyMuPDFParser
from researchmind.ingestion.models import RawPaper
from researchmind.ingestion.chunking import FixedSizeChunker
...
def ingest_paper_task(paper_url: str) -> dict:
    ...
    pdf_path = asyncio.run(HttpPDFDownloader().download(paper_id, result.pdf_url, pdf_dir))
    if pdf_path is None:
        return {"status": "error", "message": f"Failed to download PDF for {paper_id}"}

    # parse + chunk
    parsed = PyMuPDFParser().parse(Path(pdf_path), raw_paper)
